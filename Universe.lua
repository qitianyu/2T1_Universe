...
local span_16=menu.add_feature(
    "大陀螺1.6",
    "value_str",
    main_self.id,
    function(a)
...
local span_little=menu.add_feature(
    "小陀螺",
    "value_str",
    main_self.id,
    function(a)
...
span_16.threaded=true
span.threaded=true
span_little.threaded=true
span_16:set_str_data({
    "转NPC",
    "转玩家",
    "转NPC&玩家",
    "假身"
...
local rope_resolve=menu.add_feature(
    ...
    function(a)
        ...
        while a.on do
            ...
            if not entity.has_entity_collided_with_anything(my_ped) then
...
local fuck_session=menu.add_feature(
    "战局混乱",
    "toggle",
    main_net_all.id,
    function(a)
...
-----------------摇晃战局--------------------
local fuck_session2=menu.add_feature(
    "摇晃战局",
    "toggle",
    main_net_all.id,
    function(a)
...
cd_ad=menu.add_feature(
    ...
    main_network.id,
    function(a)
        menu.notify("刷屏机器人进入CD期 - 30秒","Universe",2)
        system.yield(30000)
        cd_ad2.on=false
        menu.notify("刷屏机器人已完成CD","Universe",2)
    end
)
        ...
        while a.on do
            ...
            for pid=0,31 do
                if player.is_player_valid(pid) then
                    ...
                    if player_host_info==1 then
                        msg='1. '..player_name
                    elseif player_host_info==2 then
                        msg=msg..'\n2. '..player_name
                    elseif player_host_info==3 then
                        msg2='\n3. '..player_name
                    elseif player_host_info==4 then
                        msg3='\n4. '..player_name
                    elseif player_host_info==5 then
                        msg3=msg3..'\n5. '..player_name
                    end
                end
            end
            ...
            ui.set_text_outline(true)
            ui.draw_text(msg..msg2..msg3..'\n你的序号: '..player.get_player_host_priority(player.player_id()),v2(0.8,0.3),9999)
        end
    end
)
...
-----------------反弹脚本--------------------
local anti_scrpit=menu.add_feature(
    "反弹脚本事件 Beta",
    "toggle",
    main_protect.id,
    function(a)
        menu.notify("此功能正处于测试状态，可能会有意想不到的BUG","Universe",5,6)
        if a.on then
            scrpit_hook_id=hook.register_script_event_hook(function(pid,me,script)
                ...
                if pid~=me then
                    ...
                    menu.notify("收到来自"..player.get_player_name(pid).."的脚本事件，已反弹","Universe",5)
                end
            end)
        else
...
local unlock_max_speed=menu.add_feature(
    "解锁载具速度上限",
    "toggle",
    main_vehicle_menu.id,
    function(a)
...
local vehicle_flier=menu.add_feature(
    "载具飞行",
    "slider",
    main_vehicle_menu.id,
    function(a)
...
--vehicle_flier.hidden=true

vehicle_flier.max,vehicle_flier.min,vehicle_flier.mod=1500,50,50
-------------------车载降落伞-----------------------
--------------------------------------------------
...
local make_NPC_Fire=menu.add_feature(
    "点燃NPC",
    "toggle",
    mission_cheat.id,
    function(a)
...
main_auto_skip.on=false
Anti_aim.on=true
ozark_title.on=true
host_info.on=true
Chat_trial.on=true
unlock_max_speed.on=true
...
menu.add_feature("» 给每个人分配200%分红", "action", HSCUT_CP.id, function()
...
local my_bag=menu.add_feature("» 背包容量", "action_value_i", CAYO_BAG.id, function(a)
    script.set_global_i(262145+29227,1800*a.value)
end)
my_bag.max,my_bag.min,my_bag.mod=100000,0,1
...
do
local your_cut_non_host=menu.add_feature("你的分红(非主机)", "autoaction_value_i", CAH_PLAYER_CUT.id, function(a)
    script.set_global_i(2453903 + 6494, a.value)
end)


local your_cut=menu.add_feature("你的分红", "autoaction_value_i", CAH_PLAYER_CUT.id, function(a)
    script.set_global_i(1703513 + 2326, a.value)
end)


local player2_cut=menu.add_feature("玩家2的分红", "autoaction_value_i", CAH_PLAYER_CUT.id, function(a)
    script.set_global_i(1703513 + 2326 +1, a.value)
end)


local player3_cut=menu.add_feature("玩家3的分红", "autoaction_value_i", CAH_PLAYER_CUT.id, function(a)
    script.set_global_i(1703513 + 2326 +2, a.value)
end)


local player4_cut=menu.add_feature("玩家4的分红", "autoaction_value_i", CAH_PLAYER_CUT.id, function(a)
...
local all_cuts=menu.add_feature("» 给每个人分配分红", "action_value_i", CAH_PLAYER_CUT.id, function()
    script.set_global_i(1703513 + 2326, a.value)
    script.set_global_i(1703513 + 2326 +1, a.value)
    script.set_global_i(1703513 + 2326 +2, a.value)
    script.set_global_i(1703513 + 2326 +3, a.value)
...
do
local my_cut2=menu.add_feature("你的分红", "autoaction_value_i", DDHEIST_PLYR_MANAGER.id, function(p)
...
do
local player2_cut2=menu.add_feature("玩家2的分红", "autoaction_value_i", DDHEIST_PLYR_MANAGER.id, function(p)
...
do
local player3_cut2=menu.add_feature("玩家3的分红", "autoaction_value_i", DDHEIST_PLYR_MANAGER.id, function(p)
...
do
local player4_cut2=menu.add_feature("玩家4的分红", "autoaction_value_i", DDHEIST_PLYR_MANAGER.id, function(p)
    script.set_global_i(1699568+812+50+4, p.value)
end)
player4_cut2.max,player4_cut2.min,player4_cut2.mod=100000,0,25
end

menu.add_feature("» 修改波格丹危机分红(240万/人)", "action", DDHEIST_PLYR_MANAGER.id, function()
...
do
local my_cut3=menu.add_feature("你的分红", "autoaction_value_i", CLASSIC_CUT.id, function(a)
